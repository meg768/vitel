...
`/`, `/app`, `/player/:id`, `/head-to-head/:A/:B`, `/event/:id`, `/ranking`, `/events`, `/players`, `/live`, `/live-matches`, `/log`, `/trial`, `/qna`, `/settings`, `/query/:name`, `/abc`, `/not-found`, and fallback `*` -> `NotFound`.
