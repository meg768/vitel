/*
@title
Störst fall från topp-50 (senaste 2 åren)

@description
Detta visar aktiva spelare som:
- Har haft sin högsta ranking bättre än 50
- Nådde den högsta rankingen under de senaste två åren
- Har spelat riktiga matcher (ej W/O eller RET) under de senaste två åren

Sorteras på störst skillnad mellan nuvarande ranking och högsta ranking.

Denna fråga kan vara relevant om en bra spelare inte spelat på ett
tag och därmed tappat i ranking.
...
WHERE
    ...
    AND p.highest_rank_date >= DATE_SUB(CURDATE(), INTERVAL 2 YEAR)
    AND EXISTS (
        SELECT
            1
        FROM
